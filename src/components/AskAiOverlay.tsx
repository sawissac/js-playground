...
const MarkdownWithCopy = ({ content }: { content: string }) => {
  return (
      ...
      <Markdown
        ...
        components={{
          pre({ children }: React.HTMLAttributes<HTMLPreElement>) {
            ...
            if (codeChild) {
              const codeProps = codeChild.props as { children?: React.ReactNode };
...
const SettingsPanel = ({
  ...
  const [url, setUrl] = useState(initialSettings.url || "http://localhost:11434");
  ...
  const handleFetchModels = async () => {
    ...
    setLoading(false);
    if (fetched.length === 0) {
      setError("Could not connect to Ollama or no models found. Make sure Ollama is running.");
    } else {
      setModels(fetched);
      if (!model && fetched.length > 0) setModel(fetched[0]);
    }
  };

  const handleSave = () => {
    if (!url || !model) {
      setError("Please set both URL and model.");
      return;
    }
    saveOllamaSettings({ url, model });
    setSuccess(true);
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-slate-500">
        Connect to a locally running Ollama instance to power Ask AI.
      </p>
...
export const AskAiOverlay = ({ open, onClose }: AskAiOverlayProps) => {
  ...
  const [settings, setSettings] = useState<OllamaSettings>({ url: "", model: "" });
  ...
  useEffect(() => {
    ...
    setSettings(s);
    setConfigured(isOllamaConfigured());
  }, []);
  ...
  useEffect(() => {
    if (open) {
      ...
      setSettings(s);
      setConfigured(isOllamaConfigured());
      setShowSettings(!isOllamaConfigured());
      setStreamError("");
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const gradient = ctx.createConicGradient(frame % (Math.PI * 2), w / 2, h / 2);
              ...
              {configured && (
                <div className="px-4 pb-3 flex items-center justify-between">
                  <span className="text-[10px] text-slate-400 font-mono">
                    {settings.model} · {mode === "ask" ? "Ask mode" : "Code mode"}
                  </span>
                </div>
              )}
            </>
